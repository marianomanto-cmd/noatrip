  ...
  return (
    <section id="resumen" className="scroll-mt-16 max-w-3xl mx-auto px-5 py-10">
      ...
      <p className="mt-3 text-stone-600 leading-relaxed">
        Se actualiza solo a medida que editan los días: kilómetros, noches y costos se recalculan en el momento.
      </p>

      {/* editar fechas y vehículo */}
      <div className="mt-5 flex flex-wrap items-center gap-x-5 gap-y-3 text-sm bg-white rounded-xl ring-1 ring-stone-200 p-4">
        <label className="flex items-center gap-2">
          <span className="text-stone-500 font-semibold">Desde</span>
          <input type="date" value={trip.start_date ?? ''} onChange={(e) => updateTrip({ start_date: e.target.value })} className="inp" />
        </label>
        <label className="flex items-center gap-2">
          <span className="text-stone-500 font-semibold">Hasta</span>
          <input type="date" value={trip.end_date ?? ''} onChange={(e) => updateTrip({ end_date: e.target.value })} className="inp" />
        </label>
      ...
      <p className="mt-3 text-xs text-stone-500">
        Costo total = alojamiento ({money(totals.lodging, trip.currency)}) + otros costos por día ({money(totals.extras, trip.currency)}).
        Editá precios y costos en cada jornada y este resumen se ajusta.
      </p>
    </section>
  )
}
